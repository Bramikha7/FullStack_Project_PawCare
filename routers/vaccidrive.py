...
def create_drive(drive: UpcomingVaccinationDriveBase, db: Session = Depends(get_db)):
    data = drive.model_dump()
    if 'drive_time_raw' in data:
        data['drive_time'] = data.pop('drive_time_raw')
    db_drive = UpcomingVaccinationDrive(**data)
    db.add(db_drive)
    db.commit()
    db.refresh(db_drive)
    return db_drive
...
def update_drive(drive_id: int, drive_update: UpcomingVaccinationDriveUpdate, db: Session = Depends(get_db)):
    ...
    if 'drive_time_raw' in update_data:
        update_data['drive_time'] = update_data.pop('drive_time_raw')
